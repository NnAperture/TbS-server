...
class Bytes:
    ...
    def __init__(self, value=None, id=None, init_symbol="b", cache_limit=-1, convert_block=False):
        ...
        if id is not None:
            ...
            if value is not None and len(value) > THRESHOLD and not self.convert_block:
    ...
    def set(self, value):
        if isinstance(self._obj, SimpleBytes) and len(value) > THRESHOLD and not self.convert_block:
            self._obj = LinkedBytes(value=value, init_symbol=self._init_symbol, cache_limit=self._cache_limit)
        if isinstance(self._obj, LinkedBytes) and len(value) < THRESHOLD // 2 and not self.convert_block:
            self._obj = SimpleBytes(value=value, init_symbol=self._init_symbol)
        else:
            self._obj.set(value)
